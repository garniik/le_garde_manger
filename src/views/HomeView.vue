  ...
  try {
    const res = await fetch('/api/foods', {
      ...
      body: JSON.stringify({
        name: newName.value,
        type: newType.value,
        calories: newCalories.value === '' ? null : Number(newCalories.value),
...
async function saveEdit(id) {
  ...
  try {
    const res = await fetch(`/api/foods/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Authorization: authHeader.value },
      body: JSON.stringify({
        name: editName.value,
        type: editType.value,
        calories: editCalories.value === '' ? null : Number(editCalories.value),
...
<template>
  <main class="container">
    ...
    <div v-else>
      <div v-if="listError" class="state error" style="margin-bottom: 0.5rem;">{{ listError }}</div>
      <ul class="list">
        ...
        <li v-for="f in foods" :key="f.id" class="item">
          <template v-if="editingId === f.id">
            <div class="row">
              ...
              <input v-model="editCalories" type="number" min="0" step="1" placeholder="Calories" />
            </div>
            <input v-model="editDesc" placeholder="Description" />
            <div class="actions">
              <button class="primary" :disabled="editLoading" @click="saveEdit(f.id)">{{ editLoading ? 'Enregistrement…' : 'Enregistrer' }}</button>
          ...
          <template v-else>
            ...
            <div class="actions">
              ...
              <button class="danger" @click="deleteFood(f.id)">Supprimer</button>
            </div>
          </template>
        </li>
      </ul>
...
.create { margin: 0.5rem 0 1rem; padding: 0.75rem; border: 1px solid #eee; border-radius: 8px; background: #fff; }
.create h2 { font-size: 1rem; margin: 0 0 0.5rem; }
.grid { display: grid; grid-template-columns: 1.2fr 0.8fr 0.6fr 1.4fr auto; gap: 0.5rem; align-items: center; }
.grid input, .grid select { padding: 0.5rem 0.6rem; border: 1px solid #ddd; border-radius: 6px; }
.grid button { padding: 0.5rem 0.8rem; border-radius: 6px; border: 1px solid #1b5e20; background: #2e7d32; color: #fff; cursor: pointer; }
.grid button:disabled { opacity: 0.7; cursor: default; }
.search {
  display: grid;
  gap: 0.5rem;
...
.row { display: grid; grid-template-columns: 1.2fr 0.8fr 0.6fr; gap: 0.5rem; margin-bottom: 0.5rem; }
.actions { display: flex; gap: 0.5rem; margin-top: 0.5rem; }
.actions button { padding: 0.35rem 0.7rem; border-radius: 6px; border: 1px solid #ddd; background: #fafafa; cursor: pointer; }
.actions .primary { background: #1976d2; border-color: #1565c0; color: #fff; }
.actions .danger { background: #d32f2f; border-color: #c62828; color: #fff; }
</style> 
